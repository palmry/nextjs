...
import ICON_4 from 'wsc/statics/images/halloween/icon4.svg'
import ICON_5 from 'wsc/statics/images/halloween/icon5.svg'
import ICON_6 from 'wsc/statics/images/halloween/icon6.svg'
...
const ScBackground = styled.div`
  ...
  height: 100%;
  z-index: -1;
  background-color: ${COLORS.HALLOWEEN_ORANGE};
  border-radius: 30px;
  ${MEDIA.TABLET`
    border-radius: 40px;
  `}
...
const HalloweenForm = ({ shareRef, RSVPRef, getResponseListToken, title }) => {
  ...
  const halloweenFormValidator = values => {
  ...
  const ResponseComponent = isSuccess => {
    return isSuccess === undefined ? (
      ...
      <ScResponseWrapper>
        <ScFlowerIcon src={ICON_6} />
        <ScSuccessHeader>Thank you for registering!</ScSuccessHeader>
        <ScResponseText>
          {'Please check your email to get your free scary cute Halloween printables and '}
          <ScShareLink
            onClick={() => {
    ...
    <ScWrapper>
      ...
      {submitted ? (
        ...
        <ScFlexWrapper>
          ...
          <ScHeader>
            Sign up to get your free scary cute Halloween printables for your kids to color and
            share in school!
          </ScHeader>
          <Formik
            initialValues={rsvpValues}
            validate={halloweenFormValidator}

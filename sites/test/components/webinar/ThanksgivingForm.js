...
import ICON_4 from 'wsc/statics/images/thanksgiving/icon4.svg'
import ICON_5 from 'wsc/statics/images/thanksgiving/icon5.svg'
import ICON_6 from 'wsc/statics/images/thanksgiving/icon6.svg'
...
const ScBackground = styled.div`
  ...
  height: 100%;
  z-index: -1;
  background-color: ${COLORS.THANKSGIVING_SOFT_BROWN};
  border-radius: 30px;
  ${MEDIA.TABLET`
    border-radius: 40px;
  `}
...
const ThanksgivingForm = ({ shareRef, RSVPRef, getResponseListToken, title }) => {
  ...
  const thanksgivingFormValidator = values => {
  ...
  const ResponseComponent = isSuccess => {
    return isSuccess === undefined ? (
      ...
      <ScResponseWrapper>
        <ScHatIcon src={ICON_6} />
        <ScSuccessHeader>Thank you for registering!</ScSuccessHeader>
        <ScResponseText>
          {'Please check your email to get your free Thanksgiving printables and '}
          <ScShareLink
            onClick={() => {
    ...
    <ScWrapper>
      ...
      {submitted ? (
        ...
        <ScFlexWrapper>
          ...
          <ScHeader>Sign up and receive your free printables straight in your inbox</ScHeader>
          <Formik
            initialValues={rsvpValues}
            validate={thanksgivingFormValidator}

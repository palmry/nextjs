...
import FORM from 'wsc/statics/images/fathersDay/form.svg'
import SPARKLES from 'wsc/statics/images/fathersDay/sparkles.svg'
import THANKYOU from 'wsc/statics/images/fathersDay/thankyou.svg'
...
const ScBackground = styled.div`
  ...
  height: 100%;
  z-index: -1;
  background-color: ${COLORS.FATHERS_DAY_DARK_GREY_BLUE};
  border-radius: 30px;
  ${MEDIA.TABLET`
    border-radius: 40px;
  `}
...
const FathersDayForm = ({ shareRef, RSVPRef, getResponseListToken }) => {
  ...
  const fathersDayFormValidator = values => {
  ...
  const ResponseComponent = isSuccess => {
    return isSuccess === undefined ? (
      ...
      <ScResponseWrapper>
        <ScRsvpIcon src={THANKYOU} />
        <ScSuccessHeader>Thank you for registering!</ScSuccessHeader>
        <ScResponseText>
          {'Please check your email to get your free printable Father’s Day cards and '}
          <ScShareLink
            onClick={() => {
  ...
  return (
    <ScWrapper>
      ...
      {submitted ? (
        ...
        <ScFlexWrapper>
          <ScRsvpIcon src={FORM} />
          <ScHeader>SIGN UP FOR YOUR FREE COLORING CARDS!</ScHeader>
          <Formik
            initialValues={rsvpValues}
            validate={fathersDayFormValidator}

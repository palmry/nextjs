...
const DocumentHead = props => {
  ...
  const ogDescription = props.ogDescription || props.seoDescription || APP_CONFIGS.description
  const imageUrl = get(props, 'ogImage.url') || get(props, 'image.url')
  const ogImageUrl = imageUrl
    ? imageUrl + (props.isContentfulImage ? '?w=1800&q=50&fm=jpg&fl=progressive' : '')
    : APP_CONFIGS.envUrl + APP_CONFIGS.logo
  const ogImageDescription =
    get(props, 'ogImage.title') || get(props, 'image.title', APP_CONFIGS.description)
  const ogImageWidth = get(props, 'ogImage.width') || get(props, 'image.width', 300)
  const ogImageHeight = get(props, 'ogImage.height') || get(props, 'image.height', 300)
      ...
      <Helmet
        onChangeClientState={() => {
          if (firstLoad) {
            ...
            addComscore({ catString: props.mainCategorySlug, relatedCategorySlugs })
